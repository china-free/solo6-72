...
export function ImageUploader({ compact = false }: ImageUploaderProps) {
  ...
  const { required, current, isSufficient, hint } = validationInfo;
    ...
    <div className="w-full p-4 rounded-xl border-2 border-dashed border-dark-600 bg-dark-800/50">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-display font-bold text-neon-purple flex items-center gap-2">
          ...
          自定义图片 ({current}/{required})
          <span className="text-xs font-normal text-gray-500 ml-2">（{difficulty} 难度需要 {required} 张）</span>
        </h3>
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">支持多选，{difficulty} 难度需要 {required} 张图片</p>
        </div>
      </div>
      ...
      {customImages.length > 0 && !isSufficient && (
